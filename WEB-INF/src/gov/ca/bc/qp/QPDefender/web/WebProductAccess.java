...
public class WebProductAccess extends WebInterface {
	...
	@Path("ID/{paId}")
	@RolesAllowed({MyRoles.QP_ADMIN, MyRoles.QP_SECURITY_GROUP_ADMIN})
	public Response getProductAccessById(@PathParam("paId") String productAccessId) {
		DAOAccess dao = new DAOAccess();
		Response response = null;
		try {
			response = this.getResponse(dao.lookupProductAccessById(Integer.parseInt(productAccessId)));
	...
	public Response deleteProductAccessById(@PathParam("paId") String productAccessId) {
		DAOAccess dao = new DAOAccess();
		...
		try {
			...
			this.getLogger().warn("Encoding exception when creating our url", e);
			response = Response.serverError().build();
		} finally {}
